...
nr_of_time_samples = len(df[x.quantity_mean][0])
# All Lines
g = df.groupby("cluster")
for i in range(nr_of_runs):
    model_run = df[x.quantity_mean][i]
    cluster_index = df["cluster"][i]
    axs[0].plot(range(nr_of_time_samples), model_run, alpha=0.15, color=colours[cluster_index])
...
axs[0].set_ylabel("Daily Cases", fontsize=13)
